...
def get_tickets(session: str = Cookie(None)):
    ...
    for row in c.fetchall():
        tickets.append({
            ...
            "description": clean_text(row[2] or ""),
            "status": row[3],
            "priority": row[4],
            "category": clean_text(row[5] or ""),
            "created_at": row[6],
            "satisfaction": row[7],
            "review": clean_text(row[8] or "")
        })
    conn.close()
...
def advanced_metrics(period: str = "month", operator_id: int = None, category: str = None, session: str = Cookie(None)):
    ...
    for d in op_stats.values():
        operator_stats.append({
            ...
            "politeness_avg": round(d["politeness_sum"]/d["count"], 2)
        })
    conn.close()
    return {
...
def get_categories():
    ...
    cats = []
    for row in c.fetchall():
        cats.append({"id": row[0], "name": clean_text(row[1])})
    conn.close()
    return cats
...
# ------------------------------------------------------------
# Главная страница — максимально подробный интерфейс с переключением темы
# ------------------------------------------------------------
@app.get("/")
def index():
    return HTMLResponse(clean_text("""<!DOCTYPE html>
...
<script>
    ...
    // ----- Рендер интерфейса в зависимости от роли -----
    ...
    async function renderClientTickets(container) {
        ...
        for(let t of data.tickets) {
            ...
            html += `<tr><td>${t.id}</td><td>${escapeHtml(t.title)}</td><td><span class="status-badge status-${t.status}">${t.status}</span></td><td>${t.priority}</td><td>${new Date(t.created_at).toLocaleDateString()}</td><td>${t.satisfaction?'⭐'+t.satisfaction:'—'}</td><td>${escapeHtml(t.review||'—')}</td><td><button class="bg-blue-600 text-white px-2 py-1 rounded text-sm hover:bg-blue-700" onclick="viewTicket(${t.id})">Открыть</button> ${actionBtn}</td></tr>`;
        }
                ...
                <div id="ticketModal" class="ticket-modal">
                    <div class="ticket-modal-content">
                        ...
                        <div class="space-y-2 mb-4">
                            <div><strong>Название:</strong> ${escapeHtml(ticket.title)}</div>
                            <div><strong>Описание:</strong> ${escapeHtml(ticket.description || '—')}</div>
                            ...
                            <div><strong>Категория:</strong> ${escapeHtml(ticket.category || '—')}</div>
                            <div><strong>Создана:</strong> ${new Date(ticket.created_at).toLocaleString()}</div>
                        </div>
                        <hr class="my-2">
                        <h3 class="font-semibold mb-2">Комментарии</h3>
                        <div id="commentsList" class="comments-list">
                            ${comments.map(c => `<div class="comment-item"><span class="font-semibold">${escapeHtml(c.user_name)}</span> <span class="text-xs text-gray-500">${new Date(c.created_at).toLocaleString()}</span><div class="mt-1">${escapeHtml(c.comment)}</div></div>`).join('')}
                        </div>
    ...
    async function renderOperatorTickets(container) {
        ...
        for(let t of data.tickets) {
            ...
            html += `<tr><td>${t.id}</td><td>${escapeHtml(t.title)}</td><td>${escapeHtml(t.description||'—')}</td><td><span class="status-badge status-${t.status}">${t.status}</span></td><td>${t.priority}</td><td><button class="bg-blue-600 text-white px-2 py-1 rounded text-sm hover:bg-blue-700" onclick="viewTicket(${t.id})">Открыть</button> ${actions}</td></tr>`;
        }
        html += `</tbody></table></div>`;
        container.innerHTML = html;
            ...
            let modalHtml = `
                <div id="ticketModal" class="ticket-modal">
                    <div class="ticket-modal-content">
                        ...
                        <h3 class="font-semibold mb-2">Комментарии</h3>
                        <div id="commentsList" class="comments-list">
                            ${comments.map(c => `<div class="comment-item"><span class="font-semibold">${escapeHtml(c.user_name)}</span> <span class="text-xs">${new Date(c.created_at).toLocaleString()}</span><div>${escapeHtml(c.comment)}</div></div>`).join('')}
                        </div>
                        <textarea id="newComment" rows="2" class="w-full border rounded-lg p-2 mb-2" placeholder="Напишите комментарий..."></textarea>
                        <div class="flex justify-end gap-2">
                            <button class="bg-gray-500 text-white px-4 py-2 rounded-lg hover:bg-gray-600" onclick="closeModal()">Закрыть</button>
    ...
    async function renderAdminUsers(container) {
        ...
        for(let u of users) {
            html += `<tr><td>${u.id}</td><td>${escapeHtml(u.email)}</td><td>${escapeHtml(u.full_name)}</td><td>${u.role}</td><td><select id="role-${u.id}"><option>client</option><option>operator</option><option>admin</option><option>quality</option></select></td><td><button class="bg-blue-600 text-white px-2 py-1 rounded text-sm hover:bg-blue-700" onclick="changeRole(${u.id})">Изменить</button> <button class="bg-red-600 text-white px-2 py-1 rounded text-sm hover:bg-red-700" onclick="delUser(${u.id})">Удалить</button></td></tr>`;
        }
        html += `</tbody></table></div>`;
        container.innerHTML = html;
    ...
    async function renderDashboard(container) {
        ...
        container.innerHTML = `
            <div class="card">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
                    ...
                    <div class="bg-gradient-to-br from-orange-500 to-orange-600 text-white p-4 rounded-xl shadow"><div class="text-sm opacity-90">Соблюдение SLA</div><div class="text-3xl font-bold">${m.sla_compliance}%</div></div>
                    <div class="bg-gradient-to-br from-cyan-500 to-cyan-600 text-white p-4 rounded-xl shadow"><div class="text-sm opacity-90">Ср. время решения</div><div class="text-3xl font-bold">${m.avg_resolution_minutes}<span class="text-lg"> мин</span></div></div>
                </div>
        ...
        let statusConfig = {
            labels: statusLabels.map(s=>({new:'Новые', in_progress:'В работе', resolved:'Решённые', closed:'Закрытые'}[s]||s)),
            datasets: [{ data: statusData, backgroundColor: statusLabels.map(s=>colors[s]||'#94a3b8') }]
        };
        let trendConfig = {
            labels: m.daily_labels,
            datasets: [{ label:'Заявки', data:m.daily_data, borderColor:'#3b82f6', fill: true, backgroundColor:'#3b82f620' }]
        };
        const textColor = theme === 'dark' ? '#e2e8f0' : '#1e293b';
        if(window.statusChart) window.statusChart.destroy();
        if(window.trendChart) window.trendChart.destroy();
        window.statusChartData = statusConfig;
        window.trendChartData = trendConfig;
        window.statusChart = new Chart(document.getElementById('statusChartDash'), {
            type: 'pie', data: statusConfig,
            options: { responsive: true, plugins: { legend: { labels: { color: textColor } } } }
        });
        window.trendChart = new Chart(document.getElementById('trendChartDash'), {
            type: 'line', data: trendConfig,
            options: { responsive: true, plugins: { legend: { labels: { color: textColor } } },
    ...
    async function renderAdvancedDashboard(container) {
        ...
        let users = await api('/api/users');
        let cats = await api('/api/categories');
