...
def clean_surrogates_ignore(text: str) -> str:
    """Полностью удаляет суррогатные символы"""
    return text.encode('utf-8', errors='ignore').decode('utf-8')

# По умолчанию используем замену на �
clean = clean_surrogates
...
# ------------------------------------------------------------
# Путь к базе данных (для Render /tmp, иначе локальная папка)
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# Диагностический middleware (перехватывает ошибки и выводит их в ответ)
# ------------------------------------------------------------
@app.middleware("http")
async def catch_exceptions_middleware(request: Request, call_next):
    try:
        ...
        return PlainTextResponse(clean(error_text), status_code=500)
...
def get_tickets(session: str = Cookie(None)):
    ...
    for row in c.fetchall():
        tickets.append({
            "id": row[0],
            "title": clean(row[1]),
            "description": clean(row[2]) if row[2] else "",
            "status": row[3],
            "priority": row[4],
            "category": clean(row[5]) if row[5] else "",
            "created_at": row[6],
            "satisfaction": row[7],
            "review": clean(row[8]) if row[8] else ""
        })
    conn.close()
...
def admin_logs(session: str = Cookie(None), limit: int = 100):
    ...
    logs = [{"time": row[0], "user_id": row[1], "action": row[2], "details": clean(row[3]) if row[3] else ""} for row in c.fetchall()]
    conn.close()
    return logs
...
def get_categories():
    ...
    cats = [{"id": row[0], "name": clean(row[1])} for row in c.fetchall()]
    conn.close()
    return cats
...
def export_tickets(session: str = Cookie(None)):
    ...
    for r in rows:
        writer.writerow([r[0], clean(r[1]), clean(r[2]) if r[2] else "", r[3], r[4], r[5], r[6] if r[6] else "", clean(r[7]) if r[7] else ""])
    output.seek(0)
...
def privacy_policy():
    ...
    """
    return HTMLResponse(clean(html))
...
def index():
    ...
    <script>
        ...
        async function renderAdminLogs(container) {
            ...
            for(let l of logs) html += `<tr><td data-label="Время">${new Date(l.time).toLocaleString()}<td data-label="Пользователь">${l.user_id}<td data-label="Действие">${l.action}<td data-label="Детали">${escapeHtml(l.details||'')}</tr>`;
            html += `</tbody>}</div>`;
            container.innerHTML = html;
        }
        ...
        async function renderAdvancedDashboard(container) {
            ...
            <div class="overflow-x-auto"><table class="w-full"><thead><tr><th>Оператор</th><th>Оценок</th><th>Общая</th><th>Скорость</th><th>Проф.</th><th>Вежливость</th></tr></thead><tbody id="operatorTable"></tbody>}</div>
            </div>`;
    ...
    """
    # Очистка от суррогатов перед отправкой
    return HTMLResponse(clean(html_content))
