...
<script>
    ...
    async function renderUI(){
        ...
        currentTabs=tabs;
        let tabsHtml=`<div class="flex gap-2 mb-4 border-b pb-2 flex-wrap">${tabs.map((t,i)=>`<button class="tab-btn ${i===0?'active':''}" data-tab-id="${t.id}">${escapeHtml(t.label)}</button>`).join('')}</div><div id="panes"></div>`;
