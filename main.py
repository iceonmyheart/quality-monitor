...
# ------------------------------------------------------------
# Инициализация БД (полностью идентична предыдущей версии)
# ------------------------------------------------------------
def init_db():
    try:
...
# ------------------------------------------------------------
# API (все эндпоинты такие же, как в предыдущей версии, с clean_text)
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# Главная страница с классическим переключением вкладок (без hash)
# ------------------------------------------------------------
@app.get("/")
def index():
...
<div class="max-w-7xl mx-auto px-4 py-6 container">
    <div class="flex justify-between items-center mb-8 p-4 bg-white dark:bg-gray-800 shadow rounded-2xl border">
        <div class="flex items-center gap-3"><div class="text-3xl">📊</div><div><h1 class="text-2xl font-bold text-gray-800 dark:text-white">Quality Monitor Pro</h1><p class="text-xs text-gray-500 dark:text-gray-400">Мониторинг качества услуг и технической поддержки</p></div></div>
        <div class="flex gap-4 items-center">
            ...
            <a href="/privacy" target="_blank" class="text-xs text-gray-500 dark:text-gray-400 hover:underline">Политика конфиденциальности</a>
...
<div id="qrModal" class="fixed inset-0 bg-black/70 flex items-center justify-center z-50 hidden">
    <div class="bg-white dark:bg-gray-800 rounded-2xl p-6 text-center max-w-sm">
        ...
        <img id="qrImage" src="" alt="QR Code" class="mx-auto my-4 w-48"><p class="text-sm break-all" id="qrUrl"></p>
...
<script>
    ...
    async function renderUI() {
        ...
        let tabsHtml = `<div class="flex gap-2 mb-4 border-b pb-2 flex-wrap">${tabs.map((t,i)=>`<button class="tab-btn ${i===0?'active':''}" data-tab-index="${i}">${escapeHtml(t.label)}</button>`).join('')}</div><div id="panes"></div>`;
    ...
    // ========== Рендеры (все функции полностью идентичны предыдущей рабочей версии) ==========
