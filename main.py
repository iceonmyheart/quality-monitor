...
def clean_text(text: str) -> str:
    if not text:
        return ""
    text = re.sub(r'[\ud800-\udfff]', '', text)          # удаляем суррогаты
...
# ------------------------------------------------------------
# Диагностический middleware (выводит ошибки в текст)
# ------------------------------------------------------------
@app.middleware("http")
...
# ------------------------------------------------------------
# API эндпоинты (все используют clean_text для строк)
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# Главная страница — интерфейс с переключением темы, адаптивными карточками и дашбордами
# ------------------------------------------------------------
@app.get("/")
def index():
    return HTMLResponse(clean_text("""<!DOCTYPE html>
...
<head>
    ...
    <style>
        *{transition:all 0.2s ease;}
        ...
        .tab-btn:hover{background:#15803d20;}
...
<div class="max-w-7xl mx-auto px-4 py-6 container">
    <div class="flex justify-between items-center mb-8 p-4 bg-white dark:bg-gray-800 shadow rounded-2xl border">
        ...
        <div class="flex gap-4">
            ...
            <a href="/privacy" target="_blank" class="text-xs text-gray-500 dark:text-gray-400">Политика</a>
...
<script>
    ...
    function applyTheme(){
        ...
        ['statusChartDash','trendChartDash','operatorChart'].forEach(id=>{if(window[id])window[id].destroy();});
        const activePane=document.querySelector('.tab-pane.active');
        if(activePane&&window.currentTabs){const idx=activePane.id.split('-')[1];if(window.currentTabs[idx])window.currentTabs[idx].render(activePane);}
    }
    ...
    async function loadUser(){
        try{
            ...
            document.getElementById('userPanel').innerHTML=`<span class="font-medium">${escapeHtml(currentUser.name)}</span><span class="bg-gray-200 dark:bg-gray-700 px-2 py-0.5 rounded-full text-sm">${currentUser.role}</span><button class="bg-red-50 text-red-600 hover:bg-red-100 px-3 py-1 rounded-full text-sm" onclick="logout()">Выйти</button>`;
            renderUI();
    ...
    async function renderUI(){
        ...
        let tabs=[];
        if(currentUser.role==='client')tabs=[{id:'myTickets',label:'Мои заявки',render:renderClientTickets},{id:'newTicket',label:'Новая заявка',render:renderNewTicket},{id:'knowledge',label:'База знаний',render:renderKnowledge}];
        else if(currentUser.role==='operator')tabs=[{id:'allTickets',label:'Все заявки',render:renderOperatorTickets},{id:'exportData',label:'Экспорт',render:renderExport}];
        else if(currentUser.role==='admin')tabs=[{id:'usersManage',label:'Пользователи',render:renderAdminUsers},{id:'slaSettings',label:'Настройки SLA',render:renderAdminSLA},{id:'logs',label:'Логи',render:renderAdminLogs},{id:'knowledge',label:'База знаний',render:renderKnowledge},{id:'dashboard',label:'Дашборд',render:renderDashboard},{id:'advancedAnalytics',label:'Аналитика оценок',render:renderAdvancedDashboard},{id:'exportData',label:'Экспорт',render:renderExport}];
        else if(currentUser.role==='quality')tabs=[{id:'dashboard',label:'Дашборд',render:renderDashboard},{id:'advancedAnalytics',label:'Аналитика оценок',render:renderAdvancedDashboard},{id:'knowledge',label:'База знаний',render:renderKnowledge},{id:'exportData',label:'Экспорт',render:renderExport}];
        ...
        for(let i=0;i<tabs.length;i++){let pane=document.createElement('div');pane.className=`tab-pane ${i===0?'active':''}`;pane.id=`pane-${i}`;panesDiv.appendChild(pane);}
        window.currentTabs=tabs;
        async function loadPane(idx){let pane=document.getElementById(`pane-${idx}`);if(!pane)return;let tab=tabs[idx];if(!tab)return;pane.innerHTML='';await tab.render(pane);}
        await loadPane(0);
        document.querySelectorAll('.tab-btn').forEach((btn,idx)=>{btn.onclick=async()=>{document.querySelectorAll('.tab-btn').forEach(b=>b.classList.remove('active'));btn.classList.add('active');document.querySelectorAll('.tab-pane').forEach(p=>p.classList.remove('active'));document.getElementById(`pane-${idx}`).classList.add('active');await loadPane(idx);};});
    }
    async function renderClientTickets(container){
        ...
        window.viewTicket=async(id)=>{
            let ticketsData=await api('/api/tickets');let ticket=ticketsData.tickets.find(t=>t.id===id);if(!ticket)return;let comments=await api(`/api/tickets/${id}/comments`);
        ...
        window.openDetailedReview=async(id)=>{let modal=document.createElement('div');modal.className='fixed inset-0 bg-black/70 flex items-center justify-center z-50';
    ...
    async function renderAdminLogs(container){
        ...
        let html=`<div class="card overflow-x-auto"><h3 class="text-xl font-semibold mb-4">Логи</h3><table class="w-full"><thead><tr><th>Время</th><th>Пользователь</th><th>Действие</th><th>Детали</th></tr></thead><tbody>${logs.map(l=>`<tr><td>${new Date(l.time).toLocaleString()}</td><td>${l.user_id}</td><td>${l.action}</td><td>${escapeHtml(l.details||'')}</td></tr>`).join('')}</tbody></table></div>`;
        container.innerHTML=html;
    }
    async function renderDashboard(container){
        ...
        window.statusChart=new Chart(document.getElementById('statusChartDash'),{type:'pie',data:{labels:statusLabels.map(s=>({new:'Новые',in_progress:'В работе',resolved:'Решённые',closed:'Закрытые'}[s]||s)),datasets:[{data:statusData,backgroundColor:statusLabels.map(s=>colors[s]||'#94a3b8')}]},options:{responsive:true}});
        window.trendChart=new Chart(document.getElementById('trendChartDash'),{type:'line',data:{labels:m.daily_labels,datasets:[{label:'Заявки',data:m.daily_data,borderColor:'#3b82f6',fill:true}]},options:{responsive:true}});
    }
    async function renderAdvancedDashboard(container){
        container.innerHTML=`<div class="card"><h3 class="text-xl font-semibold mb-4">Аналитика оценок</h3><div class="flex flex-wrap gap-4 mb-6"><div><label>Период</label><select id="periodFilter"><option value="month">Месяц</option><option value="week">Неделя</option><option value="quarter">Квартал</option></select></div><div><label>Оператор</label><select id="operatorFilter"><option value="">Все</option></select></div><div><label>Категория</label><select id="categoryFilter"><option value="">Все</option></select></div><button class="btn-primary" id="applyFilters">Применить</button></div><div class="grid md:grid-cols-4 gap-4 mb-6"><div class="bg-gray-100 p-4 rounded-xl"><div class="text-sm">Общая оценка</div><div id="overallAvg" class="text-2xl font-bold">-</div></div><div class="bg-gray-100 p-4 rounded-xl"><div class="text-sm">Скорость ответа</div><div id="speedAvg" class="text-2xl font-bold">-</div></div><div class="bg-gray-100 p-4 rounded-xl"><div class="text-sm">Профессионализм</div><div id="profAvg" class="text-2xl font-bold">-</div></div><div class="bg-gray-100 p-4 rounded-xl"><div class="text-sm">Вежливость</div><div id="politenessAvg" class="text-2xl font-bold">-</div></div></div><div class="mb-6"><canvas id="operatorChart" height="300"></canvas></div><div class="overflow-x-auto"><table class="w-full"><thead><tr><th>Оператор</th><th>Оценок</th><th>Общая</th><th>Скорость</th><th>Проф.</th><th>Вежливость</th></tr></thead><tbody id="operatorTable"></tbody></table></div></div>`;
