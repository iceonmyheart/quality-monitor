...
# ------------------------------------------------------------
# Главная страница с hash-навигацией и полными рендерами
# ------------------------------------------------------------
@app.get("/")
def index():
...
<script>
    ...
    function applyTheme(){
        ...
        if(activePane&&currentTabs){const idx=activePane.id.split('-')[1];if(currentTabs[idx])currentTabs[idx].render(activePane);}
    }
    ...
    async function renderUI(){
        ...
        currentTabs=tabs;
        tabIdToIndex={};tabs.forEach((t,i)=>tabIdToIndex[t.id]=i);
        let tabsHtml=`<div class="flex gap-2 mb-4 border-b pb-2 flex-wrap">${tabs.map(t=>`<button class="tab-btn" data-tab-id="${t.id}">${escapeHtml(t.label)}</button>`).join('')}</div><div id="panes"></div>`;
